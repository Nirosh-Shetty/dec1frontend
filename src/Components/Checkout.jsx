...
const Checkout = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "https://dailydish-backend.onrender.com/api/User/addStudentInformation",
        {
          customerId: user._id,
          ...newInfo,
        }
  ...
  const appLyOffferCustome = async (
    ...
    try {
      await axios.post("https://dailydish-backend.onrender.com/api/admin/createreports", {
        customerName,
        phone,
        totalOrders,
        product,
  ...
  const getSelectedAddress = async (id) => {
    ...
    try {
      let res = await axios.get(
        `https://dailydish-backend.onrender.com/api/user/getSelectedAddressByUserIDAddressID/${user?._id}/${id}`
      );
      if (res.status === 200) {
  ...
  const saveSelectedAddress = async (data) => {
    try {
      if (!user) return;
      await axios.post(`https://dailydish-backend.onrender.com/api/user/addressadd`, {
        Name: name,
        Number: mobilenumber,
        userId: user?._id,
