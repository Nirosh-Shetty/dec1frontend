...
const LocationConfirmationSimple = () => {
  ...
  const handleSearchSuggestionSelect = (place) => {
    if (placesServiceRef.current) {
      placesServiceRef.current.getDetails(
        {
          placeId: place.place_id,
  ...
  // Check if we have location data from LocationModal2 and initialize accordingly
  useEffect(() => {
    if (scriptLoaded) {
      // Check if we have location data passed from LocationModal2
      const locationState = location.state;

      if (locationState && locationState.selectedPlace) {
        // We have location data from LocationModal2
        const { location: locData, address: addr } =
          locationState.selectedPlace;

        setSelectedLocation(locData);
        setAddress(addr || "");
        validateServiceability(locData);

        // Initialize map with the selected location
        setTimeout(() => {
          if (mapRef.current) {
            initializeMap(locData);
            if (!addr) {
              getAddressFromCoordinates(locData.lat, locData.lng);
            }
          }
          setIsLoading(false);
        }, 500);
      } else {
        // No location data from modal, get current location or use default
        getCurrentLocation();
      }
    }
  }, [scriptLoaded, location.state]); // Add location.state to dependencies
  ...
  const getCurrentLocation = () => {
    if (!navigator.geolocation) {
      ...
      validateServiceability(DEFAULT_LOCATION);
      setTimeout(() => {
        if (mapRef.current) {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        // If permission denied or other error, use default Bengaluru location
        if (error.code === error.PERMISSION_DENIED) {
          setShowLocationDisabledPopup(true);
        }
...
`;
  ...
  const handleConfirmLocation = async () => {
    ...
    try {
      ...
      // If location is not serviceable, show service request modal
      else if (isServiceable === false) {
        setShowServiceablePopup(true);
        const user = JSON.parse(localStorage.getItem("user"));
        if (user) {
            ...
            {showSearch && searchSuggestions.length > 0 && (
              ...
              >
                {searchSuggestions.map((place, index) => (
                  <div
                    key={place.place_id}
                    onClick={() => handleSearchSuggestionSelect(place)}
                    style={{
                      padding: "12px 16px",
                      borderBottom:
                        index < searchSuggestions.length - 1
