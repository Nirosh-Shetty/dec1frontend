...
function CorporateDashboard() {
  ...
  const getOrders = async () => {
    try {
      let res = await axios.get(
        `https://dd-backend-3nm0.onrender.com/api/admin/getAllOrdersByCompanyId/${corporate?._id}`,
      );
      if (res.status === 200) {
  ...
  const getAllEmployees = async () => {
    try {
      let res = await axios.get(
        `https://dd-backend-3nm0.onrender.com/api/User/getUserByCompany/${corporate?._id}`,
      );
      if (res.status === 200) {
