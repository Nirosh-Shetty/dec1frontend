...
const OfferForm = () => {
  ...
  const [acquisition_channel, setAcquisitionChannel] = useState("");
  ...
  const getOffers = async () => {
    try {
      const res = await axios.get("http://localhost:7013/api/admin/offers");
      if (res.status === 200) {
  ...
  const handleAcquisitionChannelChange = (value) => {
    // Update the global acquisition_channel state
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingOffer) {
        ...
        response = await axios.post("http://localhost:7013/api/admin/offers", {
          products: formattedProducts,
          startDate,
          endDate,
          hubId: selectedHubId,
  ...
  const handleEdit = (offer) => {
    ...
    setSession(offer.session || "");
    setAcquisitionChannel(offer.acquisition_channel);
  };
  ...
  const handleDelete = async (offerId) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:7013/api/admin/offers/${offerId}`);
        getOffers();
        Swal.fire({
          title: "Deleted!",
      ...
      <form className="p-4" onSubmit={handleSubmit}>
        ...
        {products.map((product, index) => (
          <div key={index} className="border p-3 mb-3 rounded">
            ...
            {/* DELETE THIS SECTION FROM INSIDE THE PRODUCT LOOP */}
            {/* Add this AFTER the Hub/Location section and BEFORE the products mapping */}
            ...
            <div className="border p-3 mb-3 rounded bg-light">
              ...
              <div className="mb-3">
                <label className="form-label">
                  Session (Applies to all products)
                </label>
                <select
                  className="form-control"
                  value={session}
            ...
            <div className="border p-3 mb-3 rounded bg-light">
              ...
              <div className="mb-3">
                {/* <label className="form-label">Session (Applies to all products)</label> */}
                <select
                  className="form-control"
                  value={acquisition_channel}
                  onChange={(e) =>
                    handleAcquisitionChannelChange(e.target.value)
                  }
                  required
                >
          ...
          <tbody>
            {offers.map((offer, index) => (
              <tr key={index}>
                ...
                <td>{offer.session || "N/A"}</td>
                <td>{offer.acquisition_channel || "N/A"}</td>
      ...
      if (editingBanner) {
        ...
        responses = await Promise.all(
          formDataArray.map((formData) =>
            axios.post("http://localhost:7013/api/admin/banners", formData, {
              headers: { "Content-Type": "multipart/form-data" },
            }),
          ),
        );
...
const Reports = () => {
  ...
  const handleExport = async () => {
    try {
      window.location.href = "http://localhost:7013/api/admin/reports/export";
      Swal.fire({
        title: "Success",
