...
// // Your hub slot options
// const LUNCH_SLOT_OPTIONS = ["12-12:30 AM", "12:30-1:00 AM", "1:00-1:30 AM"];

// const DINNER_SLOT_OPTIONS = ["7-7:30 PM", "7:30-8:00 PM", "8:00-8:30 PM"];
...
//   // Slot-based filtering states - using your exact slot options
//   const [selectedLunchSlot, setSelectedLunchSlot] = useState("all");
//   const [selectedDinnerSlot, setSelectedDinnerSlot] = useState("all");
//   const [availableLunchSlots, setAvailableLunchSlots] = useState([]);
//   const [availableDinnerSlots, setAvailableDinnerSlots] = useState([]);
//   const [showLunchSlotFilter, setShowLunchSlotFilter] = useState(false);
//   const [showDinnerSlotFilter, setShowDinnerSlotFilter] = useState(false);
...
//   // Function to extract slot times from orders - only show slots that have orders
//   const extractSlotTimes = useCallback((ordersArray) => {
//     const lunchSlotsSet = new Set();
//     const dinnerSlotsSet = new Set();

//     ordersArray.forEach((order) => {
//       if (order.session === "Lunch" && order.lunchSlots) {
//         // Only add if it matches one of your defined lunch slot options
//         if (LUNCH_SLOT_OPTIONS.includes(order.lunchSlots)) {
//           lunchSlotsSet.add(order.lunchSlots);
//         }
//       } else if (order.session === "Dinner" && order.dinnerSlots) {
//         // Only add if it matches one of your defined dinner slot options
//         if (DINNER_SLOT_OPTIONS.includes(order.dinnerSlots)) {
//           dinnerSlotsSet.add(order.dinnerSlots);
//         }
//       }
//     });

//     // Sort slots in time order using your predefined order
//     const sortSlots = (slots, predefinedOrder) => {
...
//     return {
//       lunchSlots: sortSlots(lunchSlotsSet, LUNCH_SLOT_OPTIONS),
//       dinnerSlots: sortSlots(dinnerSlotsSet, DINNER_SLOT_OPTIONS),
//     };
//   }, []);

//   // Update available slots when orders change
//   useEffect(() => {
//     const { lunchSlots, dinnerSlots } = extractSlotTimes(orders);
//     setAvailableLunchSlots(lunchSlots);
//     setAvailableDinnerSlots(dinnerSlots);
//   }, [orders, extractSlotTimes]);

//   // Show/hide slot filters based on session selection
//   useEffect(() => {
//     if (selectedMapSession === "Lunch") {
//       setShowLunchSlotFilter(true);
//       setShowDinnerSlotFilter(false);
//       setSelectedDinnerSlot("all");
//     } else if (selectedMapSession === "Dinner") {
//       setShowLunchSlotFilter(false);
//       setShowDinnerSlotFilter(true);
//       setSelectedLunchSlot("all");
//     } else {
//       setShowLunchSlotFilter(false);
//       setShowDinnerSlotFilter(false);
//       setSelectedLunchSlot("all");
...
//   // Main filter application function - FIXED VERSION
//   const applyAllFilters = useCallback(
//     (session, hub, lunchSlot, dinnerSlot) => {
...
//       if (session !== "all") {
//         filtered = filtered.filter((order) => order.session === session);
//       }

//       // Apply hub filter
//       if (hub !== "all") {
//         filtered = filtered.filter((order) => order.hubName === hub);
//       }

//       // Apply slot filters only when a specific session AND slot are selected
//       if (session === "Lunch" && lunchSlot !== "all") {
//         filtered = filtered.filter(
//           (order) =>
//             order.session === "Lunch" && order.lunchSlots === lunchSlot,
//         );
//       } else if (session === "Dinner" && dinnerSlot !== "all") {
//         filtered = filtered.filter(
//           (order) =>
//             order.session === "Dinner" && order.dinnerSlots === dinnerSlot,
//         );
//       }
...
//       // Update map markers
//       if (clustererRef.current && mapRef.current) {
//         updateClustererMarkers(filtered);
//       }
//     },
//     [orders, updateClustererMarkers],
//   );
...
//       // Reset slot filters when session changes
//       setSelectedLunchSlot("all");
//       setSelectedDinnerSlot("all");

//       // Apply filters with reset slots
//       applyAllFilters(session, selectedHub, "all", "all");
//     },
//     [selectedHub, applyAllFilters],
//   );

//   // Slot-based filtering function
//   const filterOrdersBySlot = useCallback(
//     (sessionType, slot) => {
//       if (sessionType === "lunch") {
//         setSelectedLunchSlot(slot);
//         applyAllFilters(
//           selectedMapSession,
//           selectedHub,
//           slot,
...
//       // Ensure orders have the correct slot fields
//       const ordersWithSlots = res.data.map((order) => ({
//         ...order,
//         lunchSlots: order.lunchSlots || "",
//         dinnerSlots: order.dinnerSlots || "",
//       }));

//       const ordersWithCoordinates = ordersWithSlots.filter(
//         (order) =>
//           order.coordinates &&
//           order.coordinates.coordinates &&
//           order.coordinates.coordinates.length === 2,
...
//       applyAllFilters("all", "all", "all", "all");
...
//   // Initialize with all orders on first load
//   useEffect(() => {
//     if (orders.length > 0 && filteredOrders.length === 0) {
//       console.log("Initializing filtered orders with all orders");
//       applyAllFilters("all", "all", "all", "all");
//     }
//   }, [orders, filteredOrders.length, applyAllFilters]);
...
//       // Apply all current filters with current state
//       applyAllFilters(
//         selectedMapSession,
//         selectedHub,
//         selectedLunchSlot,
...
//             {/* NEW: Lunch Slot Filter - Only show when session is Lunch */}
//             {showLunchSlotFilter && availableLunchSlots.length > 0 && (
//               <div className="slot-selector">
//                 <label
//                   htmlFor="lunch-slot-filter"
//                   style={{
//                     fontSize: "14px",
//                     fontWeight: "600",
//                     marginRight: "8px",
...
//                 </label>
//                 <select
//                   id="lunch-slot-filter"
//                   value={selectedLunchSlot}
//                   onChange={(e) => filterOrdersBySlot("lunch", e.target.value)}
//                   style={{
//                     padding: "8px 12px",
//                     borderRadius: "6px",
//                     border: "1px solid #d1d5db",
...
//                   }}
//                 >
//                   <option value="all">All Lunch Slots</option>
//                   {availableLunchSlots.map((slot, index) => (
...
//             {/* NEW: Dinner Slot Filter - Only show when session is Dinner */}
//             {showDinnerSlotFilter && availableDinnerSlots.length > 0 && (
//               <div className="slot-selector">
//                 <label
//                   htmlFor="dinner-slot-filter"
//                   style={{
//                     fontSize: "14px",
//                     fontWeight: "600",
//                     marginRight: "8px",
...
//                 </label>
//                 <select
//                   id="dinner-slot-filter"
//                   value={selectedDinnerSlot}
//                   onChange={(e) => filterOrdersBySlot("dinner", e.target.value)}
//                   style={{
//                     padding: "8px 12px",
//                     borderRadius: "6px",
//                     border: "1px solid #d1d5db",
...
//                   }}
//                 >
//                   <option value="all">All Dinner Slots</option>
//                   {availableDinnerSlots.map((slot, index) => (
//                     <option key={index} value={slot}>
//                       {slot}
//                     </option>
//                   ))}
...
//             {/* NEW: Show active slot filters */}
//             <div className="snapshot-card">
//               <span>Active Slot</span>
//               <strong>
//                 {selectedMapSession === "Lunch" && selectedLunchSlot !== "all"
//                   ? selectedLunchSlot
//                   : selectedMapSession === "Dinner" &&
//                       selectedDinnerSlot !== "all"
//                     ? selectedDinnerSlot
//                     : "All Slots"}
//               </strong>
//             </div>
...
//                               <strong>Time Slot:</strong>
//                               <div style={{ fontSize: "13px" }}>
//                                 {selectedOrder.session === "Lunch"
//                                   ? `Lunch: ${selectedOrder.lunchSlots || selectedOrder.slot}`
//                                   : selectedOrder.session === "Dinner"
//                                     ? `Dinner: ${selectedOrder.dinnerSlots || selectedOrder.slot}`
//                                     : selectedOrder.slot || "N/A"}
//                               </div>
//                             </div>
//                             <div>
//                               <strong>Total Amount:</strong>
