...
import { BsSearch, BsPlus, BsTrash, BsCalendarDate, BsGift } from "react-icons/bs";
...
const HubWiseProductManagement = () => {
  ...
  const filteredMenuItems = useMemo(() => {
    ...
    // Filter by show offers only
    if (showOffersOnly && activeTab === "products") {
      filteredData = filteredData.filter(item => item.isOffer === true);
    }
    ...
    return filteredData;
  }, [menuItems, searchTerm, filterType, activeTab, showOffersOnly]);
  ...
  const fetchActiveOffers = async () => {
    ...
    try {
      const res = await axios.get(`${API_URL}/active-offers`, {
        params: {
          hubId: selectedHub._id,
          date: selectedDate,
        },
      ...
      setActiveOffers(res.data.offers || []);
    } catch (error) {
  ...
  const openOfferModal = (menuItem = null) => {
    if (menuItem) {
      // Edit existing offer
      setOfferForm({
        _id: menuItem._id,
        isOffer: menuItem.isOffer || false,
        offerPrice: menuItem.offerPrice || 0,
        offerStartDate: menuItem.offerStartDate 
          ? formatDateForDisplay(menuItem.offerStartDate) 
          : selectedDate,
        offerEndDate: menuItem.offerEndDate 
          ? formatDateForDisplay(menuItem.offerEndDate) 
          : selectedDate,
        customerType: menuItem.customerType || 1,
        minCart: menuItem.minCart || 1,
        productName: menuItem.productId?.foodname || "Unknown",
      });
    } else {
      // Clear form
      setOfferForm({
        ...
        isOffer: false,
        offerPrice: 0,
        offerStartDate: selectedDate,
        offerEndDate: selectedDate,
        customerType: 1,
        minCart: 1,
        productName: "",
      });
  ...
  const saveOffer = async () => {
    ...
    if (offerForm.isOffer) {
      if (!offerForm.offerPrice || offerForm.offerPrice <= 0) {
        alert("Offer price must be greater than 0.");
        return;
      }
      if (!offerForm.offerStartDate || !offerForm.offerEndDate) {
        alert("Offer start and end dates are required.");
        return;
      }
      if (new Date(offerForm.offerEndDate) < new Date(offerForm.offerStartDate)) {
    ...
    try {
      const updateData = {
        ...
        offerPrice: offerForm.isOffer ? Number(offerForm.offerPrice) : null,
        offerStartDate: offerForm.isOffer ? offerForm.offerStartDate : null,
        offerEndDate: offerForm.isOffer ? offerForm.offerEndDate : null,
        customerType: offerForm.isOffer ? Number(offerForm.customerType) : null,
        minCart: offerForm.isOffer ? Number(offerForm.minCart) : null,
      };

      const res = await axios.put(
        `${API_URL}/update/${offerForm._id}`,
        updateData,
        { headers: { Authorization: `Bearer ${token}` } },
      );

      if (res.data.success) {
        alert(offerForm.isOffer ? "Offer added successfully!" : "Offer removed successfully!");
        setShowOfferModal(false);
        await fetchHubMenu();
        await fetchActiveOffers();
      } else {
        alert(res.data.message || "Failed to save offer.");
      }
    } catch (error) {
  ...
  const removeOffer = async (menuItem) => {
    if (!window.confirm(`Remove offer from "${menuItem.productId?.foodname}"?`)) {
      return;
    }
    ...
    try {
      ...
      if (res.data.success) {
        ...
        alert(res.data.message || "Failed to remove offer.");
      }
    } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    const exportData = filteredMenuItems.map((item, index) => ({
      ...
      "Offer Start": item.offerStartDate ? formatDateForDisplay(item.offerStartDate) : "-",
      "Offer End": item.offerEndDate ? formatDateForDisplay(item.offerEndDate) : "-",
  ...
  return (
    <div className="container-fluid p-4">
      ...
      {activeOffers.length > 0 && selectedHub && (
        <Alert variant="success" className="mb-3">
          <div className="d-flex justify-content-between align-items-center">
            <div>
              <BsGift className="me-2" />
              <strong>{activeOffers.length} Active Offer(s)</strong> available for {selectedHub.hubName}
            </div>
            <Button 
              variant="outline-success" 
              size="sm"
            ...
            >
              {showOffersOnly ? "Show All Items" : "Show Offers Only"}
            </Button>
          </div>
          {activeOffers.slice(0, 3).map((offer) => (
            <div key={offer._id} className="mt-2">
              <Badge bg="warning" text="dark" className="me-2">OFFER</Badge>
              {offer.productId?.foodname} - 
              <del className="mx-1">₹{offer.hubPrice}</del>
              <strong className="text-success mx-1">₹{offer.offerPrice}</strong>
              <small className="text-muted ms-2">
                ({formatDateForDisplay(offer.offerStartDate)} to {formatDateForDisplay(offer.offerEndDate)})
              </small>
            </div>
          ))}
          {activeOffers.length > 3 && (
            <small className="text-muted">
              ...and {activeOffers.length - 3} more offers
            </small>
          )}
        </Alert>
      )}
        ...
        <Card>
          <Card.Header>
            ...
            >
              ...
              <Tab eventKey="offers" title={`Offers (${activeOffers.length})`}></Tab>
                ...
                {activeTab === "products" && (
                  <>
                    ...
                    <div className="row mb-3">
                      ...
                      <div className="col-md-2">
                        <Form.Label>Offer Filter</Form.Label>
                        <Form.Select
                          value={showOffersOnly ? "offers" : "all"}
                          onChange={(e) => setShowOffersOnly(e.target.value === "offers")}
                        >
                      ...
                      <Table striped bordered hover>
                        ...
                        <tbody>
                          {filteredMenuItems
                            ...
                            .map((item, index) => (
                              <tr key={item._id} className={item.isOffer ? "table-success" : ""}>
                                ...
                                <td>
                                  {item.isOffer ? (
                                    <>
                                      <del className="text-muted">₹{item.hubPrice}</del>
                                      <br />
                                      <strong className="text-success">₹{item.offerPrice}</strong>
                                    </>
                                  ) : (
                                    `₹${item.hubPrice}`
                                  )}
                                ...
                                <td>
                                  {item.isOffer ? (
                                    <div>
                                      ...
                                      <br />
                                      <small className="text-muted">
                                        {item.offerStartDate && formatDateForDisplay(item.offerStartDate)} - 
                                        {item.offerEndDate && formatDateForDisplay(item.offerEndDate)}
                                      </small>
                                    </div>
                                  ) : (
                                    <span className="text-muted">No offer</span>
                                  )}
                                </td>
                                ...
                                <td>
                                  <div className="d-flex gap-1 flex-wrap">
                                    ...
                                    <Button
                                      variant={item.isOffer ? "outline-warning" : "outline-success"}
                                      size="sm"
                                      onClick={() => openOfferModal(item)}
                                      title={item.isOffer ? "Edit Offer" : "Add Offer"}
                                    >
                                      <BsGift />
                                    </Button>
                                    {item.isOffer && (
                    ...
                    <div className="d-flex justify-content-between align-items-center">
                      <p className="mb-0">
                        ...
                        {showOffersOnly && ` (Offers: ${filteredMenuItems.filter(i => i.isOffer).length})`}
                      </p>
                      <ReactPaginate
                        previousLabel={"Back"}
                        nextLabel={"Next"}
                ...
                {activeTab === "offers" && (
                  <>
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      <h5 className="mb-0">
                        <BsGift className="me-2" />
                        Active Offers for <b>{selectedHub.hubName}</b>
                      </h5>
                      <Button
                        variant="outline-secondary"
                        size="sm"
                    ...
                    {activeOffers.length === 0 ? (
                      <Alert variant="info">
                        <p className="mb-0">
                          No active offers found for this hub. Go to Daily Menu to add offers to menu items.
                        </p>
                      </Alert>
                    ) : (
                      <div className="row">
                            ...
                            <Card className="border-success h-100">
                              <Card.Body>
                                <div className="d-flex justify-content-between">
                                  <Badge bg="success" className="mb-2">ACTIVE OFFER</Badge>
                                  <small className="text-muted">
                                    Min Cart: {offer.minCart || 1}
                                  </small>
                                </div>
                                <div className="d-flex align-items-center gap-3 mb-2">
                                  <img
                                    src={
                                      offer.productId?.Foodgallery?.[0]?.image2 ||
                                      "https://via.placeholder.com/50"
                                    }
                                    alt={offer.productId?.foodname}
                                    style={{ width: 60, height: 60, objectFit: "cover" }}
                                    className="rounded"
                                  />
                                  <div>
                                    <h6 className="mb-1">{offer.productId?.foodname}</h6>
                                    <small className="text-muted">
                                      {offer.menuCategory || "Menu Item"}
                                    </small>
                                  </div>
                                </div>
                                <hr />
                                <div className="d-flex justify-content-between align-items-center">
                                  <div>
                                    ...
                                    <h5 className="text-muted"><del>₹{offer.hubPrice}</del></h5>
                                  </div>
                                  <div className="text-end">
                                    <small>Offer Price</small>
                                    <h4 className="text-success">₹{offer.offerPrice}</h4>
                                  </div>
                                </div>
                                <div className="mt-2">
                                  <small className="text-muted">
                              ...
                              filteredMenuItems.map((item) => (
                                ...
                                >
                                  <span>
                                    ...
                                    {item.productId?.foodname}
                                    {item.isOffer && (
                                      <Badge bg="warning" text="dark" className="ms-2">OFFER</Badge>
                                    )}
                                  </span>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            <BsGift className="me-2" />
            {offerForm.isOffer ? "Edit Offer" : "Add Offer"}
            {offerForm.productName && ` - ${offerForm.productName}`}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group className="mb-3">
              <Form.Check
                type="switch"
                id="offer-switch"
                label={offerForm.isOffer ? "Offer Enabled" : "Offer Disabled"}
                checked={offerForm.isOffer}
                onChange={(e) =>
                  setOfferForm({ ...offerForm, isOffer: e.target.checked })
                }
              />
            </Form.Group>
              ...
              <>
                  ...
                  <div className="col-md-6">
                    <Form.Group className="mb-3">
                      ...
                      <Form.Text className="text-muted">
                        This price will be shown to eligible customers
                      </Form.Text>
                    </Form.Group>
                  </div>
                  <div className="col-md-6">
                    <Form.Group className="mb-3">
                      ...
                      >
                        <option value={1}>Normal Customers Only</option>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    min="1"
                    placeholder="Minimum items in cart"
                  />
                  <Form.Text className="text-muted">
                    Minimum number of items required in cart to avail this offer
                  </Form.Text>
                </Form.Group>
                ...
                <Alert variant="info">
                  <small>
                    ...
                    Price: ₹{offerForm.offerPrice || 0}<br />
                    Period: {offerForm.offerStartDate || "N/A"} to {offerForm.offerEndDate || "N/A"}<br />
                    For: {offerForm.customerType === 1 ? "Normal Customers" : "All Customers"}<br />
                    Min Cart: {offerForm.minCart || 1} items
                  </small>
                </Alert>
              </>
            )}
        ...
        <Modal.Footer>
          ...
          <Button
            variant={offerForm.isOffer ? "success" : "danger"}
            onClick={saveOffer}
            disabled={isLoading}
          >
            {isLoading ? (
